...
def rem_cmds(bot: Bot, update: Update, args: List[str]) -> str:
    ...
    if not args:
        ...
        if del_pref:
            update.effective_message.reply_text("നിലവിൽ @DingdiSupport കമാന്റുകൾ ഡിലീറ്റ് ചെയ്യുന്നുണ്ട്.")
        else:
            update.effective_message.reply_text("നിലവിൽ @DingdiSupport കമാന്റുകൾ ഡിലീറ്റ് ചെയ്യുന്നില്ല.")
        return ""

    if args[0].lower() in ("on", "yes"):
        sql.set_cmd_joined(str(chat.id), True)
        update.effective_message.reply_text("Aw le, @DingdiSupport hmingin a dik e😀😝!")
        return "<b>{}:</b>" \
               "\n#ANTI_COMMAND" \
               "\n<b>Admin:</b> {}" \
        ...
        update.effective_message.reply_text("I tihdan a dik lo, @DingdiSupport ah lo zawt rawh!")
        return "<b>{}:</b>" \
               "\n#ANTI_COMMAND" \
               "\n<b>Admin:</b> {}" \
        ...
        update.effective_message.reply_text("AntiCommand i dah duh chuan... 'on/yes' tih lo ziak rawh 'off/no' tiin i dah leh thei!")
        return ""
...
- /rmcmd <on/off>: when someone tries to send a @Dingdi_tgbot message, I will try to delete that!
"""
